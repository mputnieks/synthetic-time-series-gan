"""
The module converts CSV files into/from SDV using
...
class Encoder():
	...
	Values (CSV) files into SDV file to be used by the GAN
	models. The class produces the following files on encoding:
	- SDV file
	- Limits file
	- Min_max file
	...
	-----------------
		encode_train() : 
			Create the SDV encoded file from the training CSV file.

		encode_test() : 
			Create the SDV encoded file from the test CSV file using training file encoding.
	"""

	def __read_data(self, file_path, dtype=None):
		"""
		Read in the file
		"""

		data = None
		if file_path.endswith(".csv") and dtype is not None:
	...
	def __impute_column(self, df, c):
		"""
		Impute a column (c) in dataframe using LinearRegression if it is continuous or KNeighborsClassifier is it is not
		"""
		...
		# Remove columns not to be imputed
		...
		# If y is continuous (float64) then use linear regression else use KNeighborsClassifier
	...
	def access_fix_na_values(self, df, cols_ignored):return self.__fix_na_values(df, cols_ignored)

	def __truncated_beta(self, alpha, beta, low, high):
		"""
		Perform truncated beta distribution with params - alpha and beta
	...
	def __binary(self, col, limits=None):
		...
		if limits:
			...
			# Convert values that don't exist in original column to most common value
			col = col.copy()
	...
	def __save_files(self, df, prefix, limits=None, min_max=None, cols=False):
		"""
		Save the sdv, limits & min-max files
		"""
	...
	def __read_decoders(self, prefix):
		"""
	...
	def encode_test(self, 
		...
		----------
		data_file : str, required
			The test file as CSV which needs to be converted.
		encoder_file : str, required
			The CSV file that should be used for encoding this file.
		fix_na_values: boolean, optional
			Boolean variable which imputes the rows that have NA values in them (default is False.
		na_col_to_ignore: list, optional
			If the parameter "fix_na_values" is set to True, this list of columns will be ignored from imputation (default is empty list).
...
class Decoder():
	...
	def __read_decoders(self, prefix, syn_file):
		"""
		Read the decoder files
		"""
		...
		if syn_file.endswith(".csv"):
			syn = pd.read_csv(syn_file)
		else:
			syn = None

		return limits, min_max, cols, syn
	...
	def decode(self,
		...
		Parameters
		----------
		org_file : str, required
			The file to be used as reference while decoding the synthetic data file into its usable form.
		syn_file : str, required
		...
		df_converted.to_csv(syn_file[:-4] + "_decoded.csv", index=False)